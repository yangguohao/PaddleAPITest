...
class GenericRule(BaseRule):
    ...
    def apply(self, paddle_api: str) -> ConvertResult:
        ...
        if self.direct_mapping:  # 直接映射
            ...
            is_inplace = (paddle_api.endswith("_") and not paddle_api.endswith("__")) or paddle_api == "paddle.Tensor.__setitem__"
            ...
            if is_tensor_method:
                ...
                if formatted_args := self._format_args(self.torch_args, self.torch_kwargs):
                    code.append(f"result = _tmp_tensor.{torch_method}({formatted_args}, **_kwargs)")
                else:
                ...
                if formatted_args := self._format_args(self.torch_args, self.torch_kwargs):
                    code.append(f"result = {self.torch_api}({formatted_args}, **_kwargs)")
                else:
                    code.append(f"result = {self.torch_api}(**_kwargs)")    
                if is_inplace:
                    code.append(f"result = args[0] if args else next(iter(kwargs.values()))")
            ...
            for i, step in enumerate(self.composite_steps):
                ...
                code.append(f"_tmp_{i} = {step["torch_api"]}({self._format_args(torch_args, torch_kwargs)})")
